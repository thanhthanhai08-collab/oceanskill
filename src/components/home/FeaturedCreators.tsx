    ...
    <section className="border-b border-outline-variant/25 py-16 sm:py-20">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="mt-10 grid gap-5 md:grid-cols-3">
          {creators.map((creator) => <WaterTiltCard key={creator.id} className="rounded-2xl"><Link href={`/authors/${creator.id}` as "/authors"} className="group block h-full rounded-2xl border border-outline-variant/40 bg-surface-container-low/65 p-6 transition hover:border-primary/45"><div className={`grid h-16 w-16 place-items-center overflow-hidden rounded-2xl bg-gradient-to-br ${creator.glow_class}`}>{creator.avatar_url ? <img src={creator.avatar_url} alt="" className="h-full w-full object-cover" /> : <span className="material-symbols-outlined text-3xl text-white">{creator.icon}</span>}</div><h3 className="mt-5 font-geist text-xl font-semibold transition group-hover:text-primary">{creator.name}</h3><p className="mt-1 font-mono text-xs text-primary">{creator.handle}</p><p className="mt-4 text-sm leading-6 text-on-surface-variant">{creator.bio}</p><div className="mt-5 flex items-center gap-2 font-mono text-[10px] uppercase tracking-wider text-tertiary"><span className="material-symbols-outlined text-[16px]">verified</span>{t("verifiedCreator")}</div></Link></WaterTiltCard>)}
        </div>
      </div>
    </section>
  );
