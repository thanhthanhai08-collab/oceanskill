...
export interface SkillCardProps { readonly skill: SkillSummary; readonly featured?: boolean; readonly actionLabel: string; readonly categoryName?: string; }

export default function SkillCard({skill, featured = false, actionLabel, categoryName}: SkillCardProps) {
  const visual = getCategoryVisual(skill.category);
  const author = skill.authors;

  return (
    <article className={`group flex h-full flex-col rounded-xl border bg-surface-container-low/70 p-4 transition duration-300 hover:-translate-y-0.5 hover:border-primary/60 ${featured ? "border-primary/60 shadow-[0_0_22px_rgba(46,91,255,0.14)]" : "border-outline-variant/45"}`}>
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-3">
            <Link href={`/skills/category/${skill.category}`} className="truncate rounded-md border border-outline-variant/35 bg-surface-container-high/60 px-2 py-0.5 font-mono text-[10px] uppercase tracking-wider text-on-surface-variant transition hover:border-primary/45 hover:text-primary">{categoryName ?? skill.category}</Link>
            <span className="shrink-0 font-mono text-[11px] text-on-surface-variant">v{skill.current_version ?? "-"}</span>
          </div>
          <h2 className="mt-2 line-clamp-2 font-geist text-base font-semibold leading-5 tracking-tight transition group-hover:text-primary"><Link href={`/skills/${skill.slug}`}>{skill.title}</Link></h2>
        </div>
      </div>
      ...
      <div className="mt-auto flex items-center justify-between gap-3 border-t border-outline-variant/30 pt-4">
        ...
        <Link href={`/skills/${skill.slug}`} className="flex shrink-0 items-center gap-1 text-xs font-semibold text-primary">{actionLabel}<span className="material-symbols-outlined text-[16px] transition group-hover:translate-x-1">arrow_forward</span></Link>
      </div>
    </article>
  );
}
