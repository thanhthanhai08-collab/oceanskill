import type {DashboardApiKey, DashboardOrder, DashboardUsage} from "@/lib/dashboard/data";

export interface DashboardListsProps {
  readonly apiKeys: DashboardApiKey[];
...
export default function DashboardLists({apiKeys, usage, orders, locale, labels}: DashboardListsProps) {
  const formatDate = (value: string) => new Intl.DateTimeFormat(locale, {dateStyle: "medium"}).format(new Date(value));
  return (
    <div className="grid gap-6 xl:grid-cols-2">
      <section id="apiKeys" className="rounded-2xl border border-outline-variant/40 bg-surface-container-low/65 p-6">
        <h2 className="flex items-center gap-3 font-geist text-xl font-semibold"><span className="material-symbols-outlined text-primary">key</span>{labels.apiKeys}</h2>
        <div className="mt-5 space-y-3">{apiKeys.length ? apiKeys.map((key) => <div key={key.id} className="flex items-center justify-between gap-4 rounded-xl bg-surface-container p-4"><div className="min-w-0"><p className="truncate font-semibold">{key.name}</p><p className="mt-1 font-mono text-[10px] text-on-surface-variant">{key.key_prefix}••••</p></div><span className={`rounded-full px-2 py-1 font-mono text-[10px] ${key.revoked_at ? "bg-error/10 text-error" : "bg-tertiary/10 text-tertiary"}`}>{key.revoked_at ? labels.revoked : labels.active}</span></div>) : <p className="rounded-xl border border-dashed border-outline-variant/40 p-6 text-sm text-on-surface-variant">{labels.emptyKeys}</p>}</div>
      </section>
