...
export default function CreatorSkillForm({labels}: {readonly labels: CreatorSkillFormLabels}) {
  ...
  return <form action={action} className="space-y-6">
    ...
    <div className="grid gap-5 lg:grid-cols-[1fr_18rem]">
      ...
      <label className="block text-sm font-medium">{labels.fields.bundle ?? "Skill package (.zip)"}<span className="mt-2 flex min-h-36 cursor-pointer flex-col items-center justify-center rounded-xl border border-dashed border-outline-variant/60 bg-surface-container-low px-5 text-center transition hover:border-primary/60 hover:bg-primary/5"><span className="material-symbols-outlined text-3xl text-primary">folder_zip</span><span className="mt-3 text-xs leading-5 text-on-surface-variant">{labels.placeholders.bundle ?? "Optional ZIP containing one SKILL.md and its reference files. Scripts are stored and scanned, never executed."}</span><input name="bundle" type="file" accept=".zip,application/zip" className="mt-4 block w-full text-xs file:mr-3 file:rounded-lg file:border-0 file:bg-primary/10 file:px-3 file:py-2 file:font-semibold file:text-primary"/></span></label>
    </div>
