...
export default function CreatorSkillList({skills, reviewStatsBySkillId, labels, locale, ownerName, ownerAvatarUrl}: CreatorSkillListProps) {
              ...
              <div className="mt-auto border-t border-white/5 pt-3 text-xs text-on-surface-variant">
                <div className="mb-2 flex items-center justify-between gap-3">
                  <span className="flex min-w-0 items-center gap-2">
                    <span className={`relative flex h-6 w-6 shrink-0 items-center justify-center overflow-hidden rounded-full border border-white/10 bg-gradient-to-br ${visual.glowClass}`}>
                      {ownerAvatarUrl ? <Image src={ownerAvatarUrl} alt="" fill unoptimized sizes="24px" className="object-cover" /> : <span className="material-symbols-outlined text-[14px] text-white">person</span>}
                    </span>
