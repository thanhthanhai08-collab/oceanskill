  ...
  return (
    <nav className="flex flex-col gap-1" aria-label="Dashboard">
      {dashboardNavItems.map((item) => {
        ...
        return <Link key={item.label} href={item.href as "/dashboard"} onClick={onNavigate} aria-current={active ? "page" : undefined} className={`relative flex min-h-11 w-full shrink-0 items-center gap-3 rounded-lg px-3 py-3 text-sm font-semibold transition ${active ? "bg-primary/10 text-primary" : "text-on-surface-variant hover:bg-surface-container-low hover:text-on-surface"}`}><span className="material-symbols-outlined text-[19px]">{item.icon}</span><span>{labels[item.label]}</span>{active && <span className="ml-auto h-1.5 w-1.5 rounded-full bg-primary" />}</Link>;
      })}
    </nav>
  );
}
