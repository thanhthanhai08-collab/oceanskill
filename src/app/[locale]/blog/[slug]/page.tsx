  ...
  return <SiteShell>
    <article>
      <header className="border-b border-outline-variant/25"><div className="mx-auto max-w-5xl px-6 py-12 lg:px-8 lg:py-16">
        <Link href="/blog" className="inline-flex items-center gap-1 text-sm font-semibold text-primary"><span className="material-symbols-outlined text-[18px]">arrow_back</span>{t("back")}</Link>
        <div className="mt-8 flex flex-wrap items-center gap-3 font-mono text-xs text-on-surface-variant"><span className="rounded-full bg-tertiary/10 px-3 py-1 text-tertiary">{post.category}</span><span>{post.readingMinutes} {t("minuteRead")}</span></div>
