...
const plans = [
  {key: "free", credits: "50", icon: "explore", pack: null, popular: false},
  {key: "starter", credits: "200", icon: "bolt", pack: "starter_20k", popular: false},
  {key: "builder", credits: "600", icon: "rocket_launch", pack: "builder_50k", popular: true},
  {key: "power", credits: "1,400", icon: "rocket", pack: "power_100k", popular: false},
] as const;
const usageItems = ["charged", "free", "retry", "failed"] as const;
const compareRows = ["credits", "expiry", "keys", "collections", "usage"] as const;
const faqs = ["subscription", "request", "refund"] as const;
    ...
    <SiteShell>
      <main className="mx-auto max-w-7xl px-6 py-16 lg:px-8 lg:py-24">
        <header className="mx-auto max-w-4xl text-center">
          ...
          <h1 className="mt-7 font-geist text-4xl font-bold tracking-tight sm:text-6xl">{t("title")}</h1>
          <p className="mx-auto mt-6 max-w-3xl text-lg leading-8 text-on-surface-variant">{t("description")}</p>
        </header>
        ...
        <section className="mt-16 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
          {plans.map((plan) => {
            ...
            return (
              <article key={plan.key} className={`relative flex flex-col rounded-3xl border p-6 ${plan.popular ? "border-secondary/50 bg-gradient-to-b from-secondary/15 to-surface-container-low/75 shadow-[0_24px_70px_rgba(147,51,234,0.2)]" : "border-outline-variant/40 bg-surface-container-low/55"}`}>
                ...
                {plan.pack ? <Link href={{pathname: "/dashboard/billing/topup", query: {pack: plan.pack}}} className={`mt-8 flex min-h-12 items-center justify-center rounded-xl px-5 py-3 text-sm font-bold transition ${plan.popular ? "bg-secondary text-on-secondary hover:opacity-90" : "border border-primary/40 text-primary hover:bg-primary/10"}`}>{t("choosePaid")}</Link> : <Link href="/signup" className="mt-8 flex min-h-12 items-center justify-center rounded-xl bg-primary-container px-5 py-3 text-sm font-bold text-white transition hover:bg-inverse-primary">{t("chooseFree")}</Link>}
              </article>
            );
          })}
        </section>
        ...
        <section className="mt-24">
          ...
          <div className="mt-8 overflow-x-auto rounded-2xl border border-outline-variant/40">
            <table className="w-full min-w-[760px] text-left"><thead className="bg-surface-container-high/60"><tr><th className="p-5" /><th className="p-5">Free</th><th className="p-5">Starter</th><th className="p-5 text-secondary">Builder</th><th className="p-5">Power</th></tr></thead><tbody className="divide-y divide-outline-variant/30">
              {compareRows.map((row) => <tr key={row} className="bg-surface-container-low/35"><th className="p-5 text-sm font-semibold">{t(`compare.${row}`)}</th>{plans.map((plan) => <td key={plan.key} className="p-5 text-sm text-on-surface-variant">{row === "credits" ? plan.credits : row === "expiry" ? t("compare.never") : t("compare.included")}</td>)}</tr>)}
            </tbody></table>
          </div>
        </section>

        <section className="mt-24"><h2 className="font-geist text-3xl font-bold sm:text-4xl">{t("faqTitle")}</h2><div className="mt-8 grid gap-4 md:grid-cols-3">{faqs.map((faq) => <article key={faq} className="rounded-2xl border border-outline-variant/40 bg-surface-container-low/50 p-6"><h3 className="font-geist text-lg font-semibold">{t(`faq.${faq}.q`)}</h3><p className="mt-3 text-sm leading-6 text-on-surface-variant">{t(`faq.${faq}.a`)}</p></article>)}</div></section>
