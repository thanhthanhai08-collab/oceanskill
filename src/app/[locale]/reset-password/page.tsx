    ...
    <SiteShell showSearch={false}>
      <section className="relative grid min-h-[720px] place-items-center overflow-hidden px-6 py-16">
        ...
        <div className="glass-panel relative w-full max-w-md rounded-3xl p-8 shadow-2xl shadow-primary-container/10">
          <Link href="/" className="inline-flex items-center gap-1 text-sm font-semibold text-primary">
            <span className="material-symbols-outlined text-[18px]">arrow_back</span>
            {common("brand")}
          </Link>
          <h1 className="mt-7 font-geist text-3xl font-bold tracking-tight">{t("resetTitle")}</h1>
          <p className="mt-2 leading-6 text-on-surface-variant">{t("resetDescription")}</p>
          {message && (
            <p className="mt-5 rounded-xl border border-primary/25 bg-primary/10 px-4 py-3 text-sm text-primary">
              {message}
            </p>
          )}
          <form className="mt-7 space-y-4">
            <label className="block text-sm font-medium">
              ...
              <input
                ...
                minLength={6}
                required
                className="mt-2 w-full rounded-xl border border-outline-variant/50 bg-surface-container-lowest px-4 py-3 outline-none transition focus:border-primary"
