...
export interface SkillsPageProps { readonly params: Promise<{locale: string}>; readonly searchParams: Promise<{q?: string; category?: string; sort?: string}>; }
...
export default async function SkillsPage({params, searchParams}: SkillsPageProps) {
  ...
  const filtered = allSkills.filter((skill) => (category === "all" || skill.category === category) && (!query || `${skill.title} ${skill.description} ${skill.category}`.toLocaleLowerCase().includes(query)));
    ...
    <SiteShell>
      ...
      <section className="mx-auto max-w-7xl px-6 py-12 lg:px-8">
        ...
        <div className="mt-12 grid gap-10 lg:grid-cols-[260px_1fr]">
          <MarketplaceSidebar skills={allSkills} categories={categories} activeCategory={category} labels={{categories: t("categories"), allCategories: t("allCategories"), catalogRank: t("catalogRank"), trending: t("trending")}} />
          <div>
            ...
            <div className="mt-7 flex items-center justify-between gap-4">
              ...
              {category !== "all" && <span className="rounded-full bg-primary/10 px-3 py-1 text-xs text-primary">{categoryNames.get(category) ?? category}</span>}
            </div>
            {skills.length ? (
